...
interface CarServiceDao {
    ...
    @Query("SELECT * FROM car_services WHERE serviceDate >= :dateStart AND serviceDate < :dateEnd")
    suspend fun getServiceDateBetween(dateStart: Long, dateEnd: Long): List<CarServiceEntity>

    @Query("SELECT * FROM car_services WHERE nextServiceDate > 0 AND nextServiceDate >= :dateStart AND nextServiceDate < :dateEnd")
    suspend fun getNextServiceDueBetween(dateStart: Long, dateEnd: Long): List<CarServiceEntity>
}
